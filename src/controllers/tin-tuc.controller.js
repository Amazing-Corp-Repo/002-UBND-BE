...
const TinTucController = {
  ...
  async updateTinTuc(req, res) {
    ...
    let file = req.files;
    const currentUser = req.payload.userId;
    const result = await TinTucService.updateTinTuc(
      id,
      idDanhMuc,
      tieuDe,
      noiDung,
  ...
  async getAll(req, res) {
    ...
    const result = await TinTucService.getAll(
      parseInt(page),
      parseInt(size),
      idDanhMuc,
      isActive,
  ...
  async create(req, res) {
    ...
    const currentUser = req.payload.userId;
    isActive = isActive === "true" ? true : false;
    const result = await TinTucService.createTinTuc(
      idDanhMuc,
      tieuDe,
      noiDung,
      isActive,
  ...
  async updateStatus(req, res) {
    ...
    const currentUser = req.payload.userId;
    const result = await TinTucService.updateTinTucStatus(
      id,
      isActive,
      currentUser
    );
    return successResponse(
      res,
      result,
      "Cập nhật trạng thái tin tức thành công"
    );
  },

  async getTinTucToView(req, res) {
    const { id } = req.params;
    ...
    const result = await TinTucService.getTinTucToView(id, ip, deviceInfo);
    return successResponse(res, result, "Lấy tin tức để xem thành công");
  },
};
