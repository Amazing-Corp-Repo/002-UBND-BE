...
export const CreatePhanAnhPublicRequest = Joi.object({
  ...
  idTo: Joi.string().trim().uuid().required().messages({
    "string.uuid": "idTo must be a valid UUID",
...
export const ApproveOrRejectPhanAnhRequest = Joi.object({
  isApprove: Joi.boolean().required().messages({
    "boolean.base": "isApprove phải là giá trị boolean",
    "any.required": "isApprove là bắt buộc",
  }),
});
