...
const handleTrinhTuThucHienUpdate = async (
  tx,
  id_thu_tuc,
  list = [],
  currentUser = null
  ...
  if (idsToUpdate.length) {
    ...
    await Promise.all(
      itemsToUpdate.map((item) =>
        tx.trinh_tu_thuc_hien_thu_tuc.update({
          where: { id: item.id },
          data: {
...
const handleCachThucThucHienUdpate = async (
  tx,
  id_thu_tuc,
  list = [],
  currentUser = null
...
const handleLinhVucUpdate = async (
  tx,
  id_thu_tuc,
  danhSachLinhVuc = [],
  currentUser = null
...
const handleMauDonUpdate = async (
  tx,
  id_thu_tuc,
  danhSachMauDon = [],
  currentUser = null
  ...
  const itemsToUpdate = danhSachMauDon.filter((i) =>
  ...
  if (idsToUpdate.length) {
    ...
    await Promise.all(
      itemsToUpdate.map((item) =>
        tx.thu_tuc_hanh_chinh_mau_don.update({
          where: {
...
const handleThanhPhanHoSoForTruongHop = async (
  tx,
  id_truong_hop,
  list = [],
  currentUser = null
  ...
  const toDelete = existingIds.filter((id) => !incomingIds.includes(id));
  if (toDelete.length) {
    await tx.thanh_phan_ho_so.deleteMany({
      where: { id_truong_hop, id: { in: toDelete } },
    });
  }
...
const handleTruongHopUpdate = async (
  tx,
  id_thu_tuc,
  list = [],
  currentUser = null
  ...
  if (toDelete.length) {
    ...
    await tx.truong_hop_thu_tuc.deleteMany({
      where: { id_thu_tuc, id: { in: toDelete } },
    });
  }
        ...
        tx.truong_hop_thu_tuc
          ...
          .then(async (updated) => {
            ...
            if (item.thanhPhanHoSo && Array.isArray(item.thanhPhanHoSo)) {
              await handleThanhPhanHoSoForTruongHop(
                tx,
                item.id,
                item.thanhPhanHoSo,
                currentUser
  ...
  if (toAdd.length) {
    ...
    await Promise.all(
      toAdd.map(async (i) => {
        ...
        if (
          i.thanhPhanHoSo &&
          Array.isArray(i.thanhPhanHoSo) &&
          i.thanhPhanHoSo.length
        ) {
          await handleThanhPhanHoSoForTruongHop(
            tx,
            created.id,
...
const ThuTucRepository = {
  ...
  async findByMaAndTenThuTuc(maThuTuc, tenThuTuc) {
    const whereCondition = {
      is_delete: false,
      ...(maThuTuc && maThuTuc !== ""
        ? { OR: [{ ten_thu_tuc: tenThuTuc }, { ma_thu_tuc: maThuTuc }] }
  ...
  async createThuTuc(
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await handleLinhVucUpdate(
        tx,
        newThuTuc.id,
        danh_sach_linh_vuc_ids,
        currentUser
      );
      await handleMauDonUpdate(
        tx,
        newThuTuc.id,
        danh_sach_mau_don,
        currentUser
      );
      await handleCachThucThucHienUdpate(
        tx,
        newThuTuc.id,
        cach_thu_thuc_hien,
        currentUser
      );
      await handleTruongHopUpdate(
        tx,
        newThuTuc.id,
        truongHopThuTuc,
        currentUser
      );
      await handleTrinhTuThucHienUpdate(
        tx,
        newThuTuc.id,
        trinh_tu_thuc_hien,
        currentUser
  ...
  async getAllForMobile(id_linh_vuc, page, size) {
    const skip = (page - 1) * size;
    const where = {
      is_active: true,
      ...(id_linh_vuc
    ...
    return {
      thuTucs: data.map((tt) => ({
        ...
        linh_vuc: tt.thu_tuc_hanh_chinh_linh_vuc?.map(
          (l) => l.linh_vuc.ten_linh_vuc
        ),
      })),
      total,
    };
  ...
  async searchThuTucByName(search) {
    const where = {
      ...(search
        ? {
            OR: [{ ten_thu_tuc: { contains: search, mode: "insensitive" } }],
          }
        : {}),
      is_active: true,
    };
