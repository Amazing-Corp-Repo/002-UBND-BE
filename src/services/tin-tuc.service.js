...
const TinTucService = {
  ...
  async updateTinTuc(
    ...
    if (
      ...
      const hasNewImage =
        data.url_anh_dai_dien && data.url_anh_dai_dien.trim() !== "";
      const hasOldImage =
        existing.url_anh_dai_dien && existing.url_anh_dai_dien.trim() !== "";

      if (!hasNewImage && !hasOldImage) {
        throw new BaseError(
          400,
          "Ảnh đại diện tin tức là bắt buộc để xuất bản tin tức"
        );
      }
  ...
  async getAll(page, size, idDanhMuc, isActive, search) {
    const { data, totalItems } = await TinTucRepository.getAll(
      page,
      size,
      idDanhMuc,
      isActive,
  ...
  async createTinTuc(
    ...
    const existingDanhMuc = await DanhMucTinTucRepository.findById(
      idDanhMuc,
      false
    );
    if (!existingDanhMuc) {
  ...
  async getTinTucToView(id, ip, deviceInfo) {
    if (id === null || id === undefined) {
      throw new BaseError(400, "ID tin tức không được để trống");
    }
    if (deviceInfo === null || deviceInfo === undefined) {
      throw new BaseError(400, "Thông tin thiết bị không được để trống");
    }

    const result = await TinTucRepository.getTinTucAndUpdateViewCount(
      id,
      ip,
      deviceInfo
    );
