...
const ReportService = {
    ...
    async exportBaoCaoTongHopExcel(from, to) {
        ...
        // ===== SECTION: TỔNG QUAN =====
        FileService.excelStyles.sectionTitle(sheet, rowIndex++, "Tổng quan phản ánh");
        ...
        FileService.excelStyles.sectionTitle(sheet, rowIndex++, "Top 5 lĩnh vực được quan tâm");
    ...
    async getBaoCaoLinhVuc(fromRaw, toRaw) {
        ...
        for (const e of result) {
            ...
            e.ty_le = total ? Number(((e.tong_phan_anh / total) * 100).toFixed(2)) : 0;
    ...
    async exportBaoCaoLinhVucExcel(from, to) {
        ...
        const table1 = [
            ...
            ["Thời gian xử lý TB (ngày)", ...data.map(x => x.thoi_gian_xu_ly_tb_ngay)]
        ];
        ...
        const header2 = ["Lĩnh vực", ...data.map(x => x.ten_linh_vuc), "Tổng cộng"];
        FileService.excelStyles.tableHeader(sheet.addRow(header2));

        FileService.excelStyles.tableRow(
            sheet.addRow(["Số lượng phản ánh", ...data.map(x => x.tong_phan_anh), sumTotal])
        );
    ...
    async getBaoCaoTrangThai(fromRaw, toRaw) {
        ...
        for (const e of result) {
            e.ty_le = total
                ? Number(((e.so_luong / total) * 100).toFixed(2))
                : 0;
        }
    ...
    async exportBaoCaoTrangThaiExcel(fromRaw, toRaw) {
        ...
        FileService.excelStyles.sectionTitle(sheet, rowIndex++, "Số lượng phản ánh theo trạng thái");
